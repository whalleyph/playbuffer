...
int highestScore = 0; // added code for highest score
...
void MainGameEntry(PLAY_IGNORE_COMMAND_LINE)
{
	...
	Play::CreateGameObject(TYPE_AGENT8, { 155,0 }, 50, "agent8");
...
bool MainGameUpdate(float elapsedTime)
{
	...
	UpdateLasers();
	UpdateDestroyed();
	Play::DrawFontText("64px", "ARROWS KEYS TO MOVE UP AND DOWN AND SPACE TO FIRE", { DISPLAY_WIDTH / 2, DISPLAY_HEIGHT - 30 }, Play::CENTRE);
	Play::DrawFontText("132px", "SCORE: " + std::to_string(gameState.score), { DISPLAY_WIDTH / 2, 50 }, Play::CENTRE);
	Play::DrawFontText("64px", "HIGHEST: " + std::to_string(highestScore), { DISPLAY_WIDTH / 2, 100 }, Play::CENTRE);
...
void UpdateCoinsAndStars()
{
	...
	for (int id_coin : vCoins)
	{
		...
		if (Play::IsColliding(obj_coin, obj_agent8))
		{
			...
			if (gameState.agentState == STATE_PLAY) // conditional statement to make sure coins are not collected after character dies
			{
				gameState.score += 500;
